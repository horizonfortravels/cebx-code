...
class AnalyticsAuthorizationMatrixTest extends TestCase
{
    #[Test]
    public function same_tenant_with_analytics_read_gets_2xx(): void
    {
        ...
        $this->grantExternalPermissions((string) $user->id, (string) $account->id, ['analytics.read']);

        Sanctum::actingAs($user);

        $this->getJson('/api/v1/analytics/commissions')
            ->assertOk()
            ->assertJsonStructure(['data']);
    }

    #[Test]
    public function same_tenant_missing_analytics_read_gets_403(): void
    {
        $account = $this->createAccount();
        $user = $this->createUser([
            'account_id' => (string) $account->id,
    ...
    #[Test]
    public function analytics_response_does_not_leak_other_tenant_data(): void
    {
        ...
        $this->grantExternalPermissions((string) $userA->id, (string) $accountA->id, ['analytics.read']);
        ...
        $response = $this->getJson('/api/v1/analytics/commissions')
            ->assertOk();

        $shipmentsCount = (int) data_get($response->json(), 'data.shipments_count', 0);
        $this->assertSame(0, $shipmentsCount);
    }
    ...
    private function grantExternalPermissions(string $userId, string $accountId, array $permissions): void
    {
        ...
        $rolePayload = [
            ...
            'name' => 'analytics_matrix_' . Str::random(8),
            'display_name' => 'Analytics Matrix Role',
            'description' => 'Analytics authorization matrix role',
            'is_system' => false,
            'template' => null,
            'created_at' => now(),
            'updated_at' => now(),
