...
class TenantContextPermissionTest extends TestCase
{
    #[Test]
    public function test_internal_user_requires_tenancy_context_select_permission_for_tenant_context_header(): void
    {
        $tenant = $this->createAccount();

        $internalWithoutPermission = $this->createUser([
            'account_id' => null,
            'user_type' => 'internal',
        ]);

        Sanctum::actingAs($internalWithoutPermission);
    ...
    #[Test]
    public function test_internal_user_with_permission_can_resolve_explicit_tenant_context(): void
    {
        $tenant = $this->createAccount();

        $internalWithPermission = $this->createUser([
            'account_id' => null,
            'user_type' => 'internal',
        ]);

        $this->grantInternalPermission($internalWithPermission->id, 'tenancy.context.select', 'internal');

        Sanctum::actingAs($internalWithPermission);
    ...
    private function grantInternalPermission(string $userId, string $key, string $audience): void
    {
        ...
        DB::table('internal_roles')->insert([
            'id' => $roleId,
            'name' => 'tenant_context_role_'.Str::random(8),
            'display_name' => 'Tenant Context Role',
            'description' => 'Tenant context test role',
            'is_system' => false,
            'created_at' => now(),
            'updated_at' => now(),
            'deleted_at' => null,
