...
test('internal support can open the reports hub', async ({ page }) => {
  await loginWith(page, 'admin', USERS.internalSupport);
  await openReportsHub(page);

  await expect(page.locator('[data-testid="internal-report-card-shipments"]')).toBeVisible();
...
test('internal ops_readonly can open the reports hub read-only', async ({ page }) => {
...
test('external organization user is denied from the internal reports hub', async ({ page }) => {
