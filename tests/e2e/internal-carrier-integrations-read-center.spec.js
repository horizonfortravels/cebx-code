...
test('internal support can open carrier integrations list and detail', async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="internal-carrier-activity-card"]')).toBeVisible();
  await expect(page.locator('[data-testid="internal-carrier-error-card"]')).toBeVisible();
  await expect(page.locator('[data-testid="internal-carrier-credentials-card"]')).toBeVisible();
  await expect(page.locator('body')).toContainText('Carrier integration detail');
  await expect(page.locator('body')).not.toContainText('Internal Server Error');
});

test('internal ops_readonly can open carrier integrations read-only', async ({ page }) => {
...
test('carrier_manager can open carrier integrations list and detail read-only', async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="internal-carrier-health-card"]')).toBeVisible();
  await expect(page.locator('[data-testid="internal-carrier-credentials-card"]')).toBeVisible();
  await expect(page.locator('[data-testid="internal-carrier-credentials-card"]')).toContainText('API key');
  await expect(page.locator('body')).not.toContainText('Internal Server Error');
});

test('external organization user is denied from the internal carrier integrations routes', async ({ page }) => {
